...
horizontal_res = 640
#######VARIABLES####################
fcu_addr = 'tcp:127.0.0.1:5763' #'udp:127.0.0.1:14560'
##Aruco
id_to_find = 72
marker_size = 30
takeoff_height = 10
...
horizontal_fov = 69 * (math.pi / 180 ) ##Pi cam V1: 53.5 V2: 62.2
vertical_fov = 54 * (math.pi / 180)    ##Pi cam V1: 41.41 V2: 48.8
...
def lander(q):
    ...
    try:
        if ids is not None and ids[0] == id_to_find:
            ...
            if (z/conv_factor>=plnd_alt_min):
                send_land_message(x_ang,y_ang)
                print("PrecLand Active")
            else:
                print("PrecLand Stopped")

            print("X CENTER PIXEL:", x_avg, "Y CENTER PIXEL:", y_avg)
            print("FOUND COUNT:", found_count, "NOTFOUND COUNT:", notfound_count)
        ...
        print('Target likely not found. Error:', e)
        notfound_count += 1
...
with dai.Device(pipeline) as device:
    q = device.getOutputQueue("video", maxSize=2, blocking=False)
