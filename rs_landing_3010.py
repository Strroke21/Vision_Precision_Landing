...
class SafeLander(Node):
    ...
    def depth_callback(self, msg):
        ...
        # ---- LANDING ----
        central_diff = differences[valid_indices.index((2, 2))]
        if (self.current_diff <= flatness < central_diff) and (max_alt >= altitude >= final_alt) and(mode=="LAND") and (self.aruco_counter==0):
            ...
            if dist <= safe_land_radius:
                self.land_counter += 1
                self.get_logger().info(f"Landing at x: {x_dist:.2f} m, y: {y_dist:.2f} m. Distance_to_Target: {dist}")
            self.get_logger().info(f"Central Cell Difference: {central_diff}m.")
            ...
            if self.land_counter == 1:
                ...
                time.sleep(0.1)
                send_position_setpoint(vehicle, x_dist, y_dist, 0, mavutil.mavlink.MAV_FRAME_BODY_OFFSET_NED)
                time.sleep(10)
